...
def reconstruct_frame(X_hat, L0, E_u0=None, out=None, dus=None):
    if len(X_hat.shape) != 3:
        raise Exception('X_hat has wrong shape.')

    Mm = X_hat.shape[-1]

    if dus is None:
        dus = np.zeros(Mm-1, dtype=X_hat.dtype)
        dus[:] = get_du(Mm, L0)

    if out is None:
        out = np.zeros((4, 4, Mm), dtype=X_hat.dtype)
        out[0,0,0] = 1
    ...
    _th = np.zeros((3, Mm-1))
    _se3_X = construct_se3_elem(_th, X_hat[...,:-1])

    exp_se3_X = compute_exp_se3_variable_d(_se3_X, dus)

    for i in range(1, Mm):
        out[:,:,i] = out[:,:,i-1].dot(exp_se3_X[:,:,i-1])
...
def integrate_frame_forward(Fr, N, dt, N_is_matrix=False):
    if N_is_matrix:
        N_hat = N
    else:
        N_hat = hat_vec_to_mat(N)
    V = np.zeros(3)
    Y = construct_se3_elem(V, N_hat)
...
def reconstruct_curve(c_X, Fr_u0, Mm, Mm_save, path_handler_render, pre_transform=None):
    Mm_render = path_handler_render.Mm
    L0 = path_handler_render.L

    #print(Mm, Mm_render, c_X.shape)
    c_X_render = path_handler_render.change_Mm(c_X, Mm, Mm_render)
    X_render = path_handler_render.iDT(c_X_render)

    if not pre_transform is None:
        X_render = pre_transform(X_render)
    
    R_u0, E_u0 = get_R_and_frame(Fr_u0)
    R, E = reconstruct_frame(hat_vec_to_mat(X_render), L0, E_u0)
...
def reconstruct_curve_cheb(X, Fr_u0, Mm_render, Mm_save, path_handler, pre_transform=None):
    L0 = path_handler.L

    X_render = path_handler.cheb_to_unif(X, Mm_render)
