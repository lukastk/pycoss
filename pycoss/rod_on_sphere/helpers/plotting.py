...
def plot_centerline_and_frame(E, fig=None, ax=None, N_frame=15, frame_scale=0.1, from_top=False, perspective=True):
    if fig is None:
        fig = plt.figure(figsize=(10,10))
    if ax is None:
        ax = plt.axes(projection='3d')
    ...
    r = 0.985
    ax.plot_wireframe(x*r, y*r, z*r, linewidth=0.5, cstride=stride, rstride=stride, color=(0,0,0,0.3))
...
def vid_centerline_and_frame_3D(Es, fig=None, ax=None, save_name=None, frame_scale=0.1, N_frame=15, from_top=False, perspective=True):
    if fig is None:
        fig = plt.figure(figsize=(10,10))
    if ax is None:
        ax = plt.axes(projection='3d')
    ...
    ani = animation.FuncAnimation(
        fig, animate, interval=1, blit=True, save_count=Es.shape[0])
