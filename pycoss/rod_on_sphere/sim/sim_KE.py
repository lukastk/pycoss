...
def KE_simulate(params):
    ...
    if N_save != -1:
        ...
        if type(path_handler) == pycoss.rod.interp.cheb.ChebHandler:
            R0, E0, close_err0 = reconstruct_curve_cheb(X0, Fr_u0, Mm_render, Mm_save, path_handler, pre_transform=pre_transform_render)
        else:
            R0, E0, close_err0 = reconstruct_curve(c_X, Fr_u0, Mm, Mm_save, path_handler_render, pre_transform=pre_transform_render)
        saved_R[0] = R0
        saved_E[0] = E0
    ...
    for sim_n in range(1, Nt):
        ...
        X, N = integrator(c_X, sim_t, dt, compute_N, path_handler, lmbd, taylor_tol,
        ...
        if N_save != -1 and sim_n % save_per_N == 0:
            ...
            if type(path_handler) == pycoss.rod.interp.cheb.ChebHandler:
                R, E, close_err = reconstruct_curve_cheb(X, Fr_u0, Mm_render, Mm_save, path_handler, pre_transform=pre_transform_render)
            else:
                R, E, close_err = reconstruct_curve(c_X, Fr_u0, Mm, Mm_save, path_handler_render, pre_transform=pre_transform_render)
            saved_R[save_k] = R
            saved_E[save_k] = E
    ...
    if type(path_handler) == pycoss.rod.interp.cheb.ChebHandler:
        Rf, Ef, close_err_f = reconstruct_curve_cheb(X, Fr_u0, Mm_render, Mm_save, path_handler, pre_transform=pre_transform_render)
    else:
        Rf, Ef, close_err_f = reconstruct_curve(c_X, Fr_u0, Mm, Mm_save, path_handler_render, pre_transform=pre_transform_render)
