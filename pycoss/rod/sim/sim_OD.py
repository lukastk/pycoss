...
def OD_simulate(params):
    ...
    compute_F = params['F']
    compute_M = params['M']
    compute_eF = params['eF'] if 'eF' in params else None
    compute_eM = params['eM'] if 'eM' in params else None
    ...
    for sim_n in range(0, Nt):
        ...
        th, pi, V, Omg, F, M, eF, eM = integrator(c_th, c_pi, sim_t, dt, compute_F, compute_M, path_handler, lmbd, taylor_tol,
                    pre_transform=pre_transform, post_transform=post_transform, compute_eF=compute_eF, compute_eM=compute_eM)
