...
def hat_vec_to_mat(vec, out=None):
    ...
    if out is None:
        if len(vec.shape) == 3:
            out = np.zeros((3,3,vec.shape[-2],vec.shape[-1]), dtype=vec.dtype)
        else:
...
def hat_mat_to_vec(mat, out=None):
    if out is None:
        if len(mat.shape) == 4:
            out = np.zeros((3,mat.shape[-2],mat.shape[-1]), dtype=mat.dtype)
        else:
...
def sqr_norm(vec, out=None):
    return np.einsum('iuv,iuv->uv', vec, vec, out=out)
...
def so3_norm(w, out=None):
    if len(w.shape) == 4:
        if out is None:
            out = np.zeros(w.shape[-2:], dtype=w.dtype)
...
def vec_vec_dot(vec1, vec2, out=None):
    return np.einsum('iuv,iuv->uv', vec1, vec2, out=out)

def mat_vec_dot(mat, vec, out=None):
    return np.einsum('ijuv,juv->iuv', mat, vec, out=out)

def mat_mat_dot(mat1, mat2, out=None):
    return np.einsum('ijuv,jkuv->ikuv', mat1, mat2, out=out)
...
def cov_derivv(pi, th, path_handler, out=None):
    _cov_deriv_buffer = get_buffers(th.shape, 1, dtype=th.dtype)

    if out is None:
        out = np.zeros(th.shape, dtype=th.dtype)

    path_handler.diffv_f(th, out=out)
    cross(pi, th, out=_cov_deriv_buffer)
    out += _cov_deriv_buffer
...
def construct_oriented_frame(R, E, out=None):
    if out is None:
        if len(R.shape) == 3:
            out = np.zeros((4,4, R.shape[-2], R.shape[-1]), dtype=R.dtype)
        else:
...
def construct_se3_elem(th, pi_hat, out=None):
    if out is None:
        if len(th.shape) == 3:
            out = np.zeros((4, 4, th.shape[-2], th.shape[-1]), dtype=th.dtype)
        else:
...
def compute_exp_se3_d_A_matrix(w, d, taylor_tol=1e-2, out=None, w_norm=None, psi=None, taylor_mask=None):
    ...
    if len(w.shape) == 4:
        if out is None:
            out = np.zeros((3,3,w.shape[-2],w.shape[-1]), dtype=w.dtype)

        N_taylor = np.count_nonzero(taylor_mask)
        taylor_ratio = N_taylor/(w.shape[-1] * w.shape[-2])
...
def compute_exp_se3_d_A_matrix_analytic(w, d, out=None, w_norm=None, psi=None):
    if len(w.shape) != 3 and len(w.shape) != 4:
        w = w.reshape((3,3,1,1))
        reshape_out = True
    else:
        reshape_out = False
    ...
    if len(w.shape) == 4:
        np.einsum('ijuv,jkuv->ikuv',_buffer_nw,_buffer_nw, out=_buffer_nw_sqrd)
    else:
...
def compute_exp_se3_d_A_matrix_taylor(w, d, out=None, w_norm=None, psi=None):
    ...
    if len(w.shape) == 3:
        ...
        np.einsum('ijuv,jkuv->ikuv',dw,dw, out=dw_sqrd)
...
def compute_exp_se3(X, d, taylor_tol=1e-2, out=None, w_norm=None, psi=None, taylor_mask=None):
    ...
    if len(X.shape) == 4:
        if out is None:
            out = np.zeros((4,4,w.shape[-2],w.shape[-1]), dtype=w.dtype)
...
def compute_exp_se3_analytic(X, d, out=None, w_norm=None, psi=None):
    ...
    out[0,0] = 1
    if len(w.shape) == 4:
        np.einsum('ijuv,juv->iuv', d_A, th, out=out[1:,0])
    else:
...
def compute_exp_adj_so3_q(Omg, du_Omg, V, dt, taylor_tol=1e-2, out=None, w_norm=None, psi=None, taylor_mask=None):
    ...
    if len(Omg.shape) == 3:
        ...
        N_taylor = np.count_nonzero(taylor_mask)
        taylor_ratio = N_taylor/(out.shape[-1] * out.shape[-2])
        if taylor_ratio > 0.5:
...
def compute_exp_se3_variable_d(X, d, taylor_tol=1e-2, out=None, w_norm=None, psi=None, taylor_mask=None):
    ...
    if len(X.shape) == 3:
        if out is None:
            out = np.zeros((4,4,w.shape[-2],w.shape[-1]), dtype=X.dtype)
...
def reconstruct_surface(thu, thv, piu, piv, Fr_uv0, Mmu_render, Mmv_render, Mmu_save, Mmv_save, path_handler, pre_transform=None):
    R_save = np.zeros((3, Mmu_save, Mmv_save))
    E_save = np.zeros((3, 3, Mmu_save, Mmv_save))

    path_handler_1d = rod.interp.cheb.ChebHandler(3, path_handler.Nmu, path_handler.Nmu, path_handler.Lu, mpmath_dps=-1)
    Rv0_save, Ev0_save, _ = rod.helpers.reconstruct_curve_cheb(thu[...,:,0], piu[...,:,0], Fr_uv0, Mmu_render, Mmu_save, path_handler_1d, pre_transform=pre_transform)

    R_save[...,:,0] = Rv0_save
    E_save[...,:,0] = Ev0_save

    thv_render = path_handler.cheb_to_unif(thv, Mmu_unif=Mmu_save, Mmv_unif=None)
    piv_render = path_handler.cheb_to_unif(piv, Mmu_unif=Mmu_save, Mmv_unif=None)

    path_handler_1d = rod.interp.cheb.ChebHandler(3, path_handler.Nmv, path_handler.Nmv, path_handler.Lv, mpmath_dps=-1)
    
    for i in range(Mmu_save):
        Fr_u0 = np.zeros((4,4))
        Fr_u0[1:,0] = R_save[...,i,0]
        Fr_u0[1:,1:] = E_save[...,i,0]
        
        Rui_save, Eui_save, _ = rod.helpers.reconstruct_curve_cheb(thv_render[...,i,:], piv_render[...,i,:], Fr_u0, Mmv_render, Mmv_save, path_handler_1d, pre_transform=pre_transform)
        R_save[...,i,:] = Rui_save
        E_save[...,i,:] = Eui_save

    return R_save, E_save