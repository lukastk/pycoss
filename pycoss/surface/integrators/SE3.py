...
def integrator_KE_SE3(thu, thv, piu, piv, t, dt, compute_V, compute_Omg, path_handler, taylor_tol, pre_transform=None, post_transform=None):
    f_buffer = path_handler.f_buffer
    f_shape = path_handler.f_shape
    Mmu = path_handler.Nmu
    Mmv = path_handler.Nmv

    V, Omg, p, q, du_V, du_Omg, _d_A_dot_N, f_buffer2  = get_buffers(f_shape, 8)
    ...
    du_V = path_handler.diffu_f(V, out=du_V)
    ...
    f_buffer2 = mat_vec_dot(B, piu, out=f_buffer2)
    ...
    dv_V = path_handler.diffv_f(V, out=du_V)
    
    q = compute_exp_adj_so3_q(Omg, dv_Omg, V, dt, w_norm=Omg_norm, psi=psi, taylor_mask=taylor_mask, out=q)

    f_buffer = mat_vec_dot(d_A, V, out=f_buffer)
    f_buffer2 = mat_vec_dot(B, piv, out=f_buffer2)
    p = cross(f_buffer, f_buffer2, out=p)

    mat_vec_dot(B, thv, out=thv)
    thv[:] -= p + q
    mat_vec_dot(d_A, dv_V, out=_d_A_dot_N)
    thv[:] += _d_A_dot_N
